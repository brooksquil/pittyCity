...
const countdownInit = () => {
  ...
  countdownElements.forEach(el => {
    ...
    const countDownDate = new Date(`${userOptions?.month} ${userOptions?.date + ','} ${userOptions.year}`).getTime(); // Update the count down every 1 second

    const updateCountdown = setInterval(() => {
      const currentTime = new Date().getTime();
      const distance = countDownDate - currentTime; // Time calculations for days, hours, minutes and seconds

      const days = Math.floor(distance / (1000 * 60 * 60 * 24));
      const hours = Math.floor(distance % (1000 * 60 * 60 * 24) / (1000 * 60 * 60));
      const minutes = Math.floor(distance % (1000 * 60 * 60) / (1000 * 60));
      const seconds = Math.floor(distance % (1000 * 60) / 1000);
      countdownElement.innerHTML = `${days} days ${('0' + hours).toString().slice(-2)}:${('0' + minutes).toString().slice(-2)}:${('0' + seconds).toString().slice(-2)}`; // If the count down is over, write some text 
...
const fancyNavInit = () => {
  ...
  if (isFancynavbarTop) {
    ...
    const childAnim = { ...anim,
      delay: 0.4
    };
    const addonAnim = { ...anim,
      delay: 0.5
    };
  ...
  if (fancynavbar) {
    ...
    const fancyNavItems = document.querySelectorAll(`${Selector.FANCYNAV_LINK}, ${Selector.FANCY_DROPDOWN_MENU}`); //$fancyNavItems.css('opacity', 0);
    ...
    fancyNavLinks.forEach(fancyNavLink => {
      fancyNavLink.addEventListener(Events.CLICK, e => {
        const fancyLink = e.target; // if one-page
...
const swiperInit = () => {
  ...
  swipers.forEach(swiper => {
    ...
    const newSwiper = new window.Swiper(swiper, { ...options,
      navigation: {
